...
# Varmistetaan, että percentage_power on saatavilla ennen rollmeanin käyttöä
monthly_power_individual_avg <- monthly_power_individual %>%
  group_by(Holder) %>%
  arrange(month) %>%
  mutate(avg_power_last_5_years = rollmean(percentage_power, k = 60, fill = NA, align = "right")) %>%
  ungroup()
