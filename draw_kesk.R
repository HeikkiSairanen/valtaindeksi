...
# Suodatetaan vain KESK
kesk_power <- monthly_power %>%
  filter(Party == "KESK")

# Visualisoidaan ggplot2:lla
ggplot(kesk_power, aes(x = month, y = percentage_power, color = Party, group = Party)) +
  geom_line(color = "#95C11A") +
  labs(title = "KESK:n valta Suomessa ajan funktiona",
       x = "Vuosi",
